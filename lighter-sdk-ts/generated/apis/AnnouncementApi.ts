...
export class AnnouncementApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * announcement
     */
    public async announcement(_options?: Configuration): Promise<RequestContext> {
...
export class AnnouncementApiResponseProcessor {
     ...
     */
     public async announcementWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Announcements >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Announcements = ObjectSerializer.deserialize(
